...
public class SaOAuthClientController {
	...
	// 相关参数配置 
	private String clientId = "1002";								// 应用id
	private String clientSecret = "aaaa-bbbb-cccc-dddd-ffff";		// 应用秘钥
	...
	public SaResult getUserinfo2(String code) {
		// 调用Server端接口，获取 Access-Token 以及其他信息
		String str1 = OkHttps.sync(serverUrl + "/oauth2/token")
				.addBodyPara("grant_type", "authorization_code")
				.addBodyPara("code", code)
				.addBodyPara("client_id", clientId)
				.addBodyPara("client_secret", clientSecret)
				...
				.getBody()
				.toString();
		SoMap so = SoMap.getSoMap().setJsonString(str1);
		System.out.println("返回结果: " + so);
		...
		String str = OkHttps.sync(serverUrl + "/oauth2/userinfo")
				.addBodyPara("access_token", access_token)
				.post()
				.getBody()
				.toString();
		SoMap so2 = SoMap.getSoMap().setJsonString(str);
		System.out.println("返回结果: " + so2);

		// code不等于200  代表请求失败
		if(so2.getInt("code") != 200) {
			return SaResult.error(so2.getString("msg"));
		}

		// 返回相关参数 (data=获取到的资源 )
		SoMap data2 = so2.getMap("data");
		return SaResult.data(data2);
	}
